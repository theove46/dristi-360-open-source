...
class SliderDataSourceImp extends BaseRemoteSource implements SliderDataSource {
  ...
  SliderDataSourceImp({required this.cacheService});
  ...
  Future<DestinationsListResponseModel> sliderComponents({
    ...
    final cachedData = await cacheService.getSliderData();
    ...
    try {
      ...
      //     '${DioProvider.baseUrl}${API.components}${API.language}$appLanguage${API.sliders}';
      // final Future<Response> dioCall = dioClient.get(endpoint);
      // final Response response = await callApiWithErrorParser(dioCall);

      String endpoint =
          appLanguage == 'en' ? Assets.slidersEn : Assets.slidersBn;
